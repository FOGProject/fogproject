...
- **Mass edit on the host list**, and it asks before it overwrites. Tick any
  ...
  *No change / Set on all / Clear on all* control. Nothing is written unless
  ...
  auto-logout. It also covers the rest of a host's General tab, which the
  group page never pushed: description, architecture, desired agent version
  and the Secure Boot enrollment record. Each of those saves through the same
  checks as the host page, so a value the host page refuses is refused here
  before any host is written. The hostname is the one field left out, because
  no single name fits two hosts.
