...
class FOGConfigurationPage extends FOGPage {
            ...
            foreach((array)$_FILES['module']['tmp_name'] AS $index => &$tmp_name) {
                if (file_exists($_FILES['module']['tmp_name'][$index])) {
                    ...
                    if(file_get_contents($_FILES['module']['tmp_name'][$index])) {
                        ...
                        if ($ClientUpdater->save()) $this->FOGCore->setMessage('Modules Added/Updated!');
                    }
                }
            }
            unset($tmp_name);
        ...
        foreach ((array)$ServiceCats AS &$ServiceCAT) {
            ...
            foreach ((array)$ServMan AS &$Service) {
                if ($Service->get('name') == 'FOG_PIGZ_COMP') $type = '<div id="pigz" style="width: 200px; top: 15px;"></div><input type="text" readonly="true" name="${service_id}" id="showVal" maxsize="1" style="width: 10px; top: -5px; left:225px; position: relative;" value="${service_value}" />';
                    ...
                    foreach(array('ipxe.pxe','ipxe.kpxe','ipxe.kkpxe','undionly.pxe','undionly.kpxe','undionly.kkpxe','ipxe.efi','snp.efi','snponly.efi') AS $viewop) $options[] = '<option value="'.$viewop.'"'.($Service->get('value') == $viewop ? 'selected="selected"' : '').'>'.$viewop.'</option>';
                    $type = "\n\t\t\t".'<select name="${service_id}" style="width: 220px" autocomplete="off">'."\n\t\t\t\t".implode("\n",$options)."\n\t\t\t".'</select>';
                    unset($options);
                    ...
                    if ($this->FOGCore->getSetting('FOG_QUICKREG_IMG_ID') > 0) $Image = new Image($this->FOGCore->getSetting('FOG_QUICKREG_IMG_ID'));
    ...
    public function settings_post() {
        ...
        foreach ((array)$ServiceMan AS &$Service) {
            ...
            if ($Service->get('name') == 'FOG_MEMORY_LIMIT' && ($_REQUEST[$key] < 128 || !is_numeric($_REQUEST[$key]))) $Service->set('value',128)->save();
            else if ($Service->get('name') == 'FOG_QUICKREG_IMG_ID' && empty($_REQUEST[$key])) $Service->set('value',-1)->save();
            else if ($Service->get('name') == 'FOG_USER_VALIDPASSCHARS') $Service->set('value',addslashes($_REQUEST[$key]))->save();
            else if ($Service->get('name') == 'FOG_AD_DEFAULT_PASSWORD') $Service->set('value',$this->encryptpw($_REQUEST[$key]))->save();
                ...
                } else $Service->set('value',0)->save();
            } else if ($Service->get('name') == 'FOG_MULTICAST_ADDRESS') {
                if (filter_var($_REQUEST[$key], FILTER_VALIDATE_IP)) $Service->set('value',$_REQUEST[$key])->save();
                else $Service->set('value',0)->save();
