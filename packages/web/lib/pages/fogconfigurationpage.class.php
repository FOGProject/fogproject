...
class FOGConfigurationPage extends FOGPage {
    ...
    public function pxemenu() {
        ...
        $bootKeys = static::getClass('KeySequenceManager')->buildSelectBox($this->getSetting('FOG_KEY_SEQUENCE'));
    ...
    public function client_updater_post() {
        if ($_REQUEST['delcu']) {
            static::getClass('ClientUpdaterManager')->destroy(array('id'=>$_REQUEST['delcu']));
        ...
        try {
            ...
            foreach ((array)$_FILES['module']['tmp_name'] AS $index => &$tmp_name) {
                ...
                static::getClass('ClientUpdater',@max(static::getClass('ClientUpdater',array('name'=>$filename),'id')))
                    ->set('name',$filename)
                    ->set('md5',$md5)
            ...
            foreach ((array)static::getClass('ServiceManager')->find(array('category'=>$ServiceCAT),'AND','id') AS $i => &$Service) {
                ...
                case 'FOG_DEFAULT_LOCALE':
                    ...
                    foreach ((array)static::$foglang['Language'] AS $lang => &$humanreadable) {
                        printf('<option value="%s"%s>%s</option>',$lang,($this->getSetting('FOG_DEFAULT_LOCALE') == $lang || $this->getSetting('FOG_DEFAULT_LOCALE') == static::$foglang['Language'][$lang] ? ' selected' : ''),$humanreadable);
                        unset($humanreadable);
                    }
                    $type = sprintf('<select name="${service_id}" autocomplete="off" style="width: 220px">%s</select>',ob_get_clean());
                    break;
                case 'FOG_QUICKREG_IMG_ID':
                    $type = static::getClass('ImageManager')->buildSelectBox($this->getSetting('FOG_QUICKREG_IMG_ID'),sprintf('%s" id="${service_name}"',$Service->get('id')));
                    break;
                case 'FOG_QUICKREG_GROUP_ASSOC':
                    $type = static::getClass('GroupManager')->buildSelectBox($this->getSetting('FOG_QUICKREG_GROUP_ASSOC'),$Service->get('id'));
                    break;
                case 'FOG_KEY_SEQUENCE':
                    $type = static::getClass('KeySequenceManager')->buildSelectBox($this->getSetting('FOG_KEY_SEQUENCE'),$Service->get('id'));
                    break;
                case 'FOG_QUICKREG_OS_ID':
                    $ImageName = _('No image specified');
                    if ($this->getSetting('FOG_QUICKREG_IMG_ID') > 0) $ImageName = static::getClass('Image',$this->getSetting('FOG_QUICKREG_IMG_ID'))->get('name');
