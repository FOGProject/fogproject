...
class ImageManagementPage extends FOGPage
{
    ...
    public function __construct($name = '')
    {
        ...
        self::$returnData = function (&$Image) use (&$servSize) {
            ...
            $imageSize = $this->formatByteSize(array_sum(explode(':', $Image->get('size'))));
            $path = $Image->get('path');
    ...
    public function edit()
    {
        ...
        if ($_SESSION['FOG_FORMAT_FLAG_IN_GUI']) {
            $format = sprintf('<select name="imagemanage"><option value="1"%s>%s</option><option value="0"%s>%s</option></select>', $this->obj->get('format') ? ' selected' : '', _('Partimage'), !$this->obj->get('format') ? ' selected' : '', _('Partclone'));
        }
        $fields = array(
        ...
        unset($this->data);
        if ($GroupDataExists) {
            printf('<br/><p class="c"><input type="submit" value="%s"/></p></form></p>', _('Add Image to Group(s)'));
        }
        $this->headerData = array(
            '<input type="checkbox" name="toggle-checkbox" class="toggle-checkboxAction"/>',
            '',
        ...
        $this->render();
        if (count($this->data) > 0) {
            printf('<p class="c"><input name="update" type="submit" value="%s"/>&nbsp;<input name="deleteGroup" type="submit" value="%s"/></p>', _('Update Primary Group'), _('Delete Selected Group associations'));
        }
        echo '</form></div></div>';
    }
    ...
    public function multicastPost()
    {
        try {
            ...
            if ($countmc >= $countmctot) {
                throw new Exception(sprintf(_('Please wait until a slot is open<br/>There are currently %s tasks in queue<br/>Your server only allows %s'), $countmc, $countmctot));
            }
