...
class TaskManagementPage extends FOGPage
{
	...
	public function index()
	{
		...
		// Find data -> Push data
		foreach ($this->FOGCore->getClass('TaskManager')->find(array('stateID' => array(1,2,3))) AS $Task)
		{
	...
	public function search_post()
	{
		...
		// Find data -> Push data
		foreach ($this->FOGCore->getClass('TaskManager')->search($keyword) AS $Task)
		{
			$Host = current($this->FOGCore->getClass('HostManager')->find(array('id' => $Task->get('hostID'))));
			$this->data[] = array(
				'columnkill' => $Task->get('stateID') == 1 || $Task->get('stateID') == 2 || $Task->get('stateID') == '3' ? '${details_taskforce} <a href="?node=tasks&sub=cancel-task&id=${id}"><span class="icon icon-kill" title="' . _('Cancel Task') . '"></span></a>' : '',
				...
				'type'	=> $Task->getTaskTypeText(),
				'percentText' => $Task->get('percent'),
				'tablerow' => ($Task->get('percent') && $Task->get('stateID') == 3 ? '<tr id="progress-${host_id}" class="${class}"><td colspan="7" class="task-progress-td min"><div class="task-progress-fill min" style="width: ${width}px"></div><div class="task-progress min"><ul><li>${elapsed}/${remains}</li><li>${percent}%</li><li>${copied} of ${total} (${bpm}/min)</li></ul></div></td></tr>' : ''),
				'class' => ++$i % 2 ? 'alt2' : 'alt1',
				'width' => 600 * ($Task->get('percent')/100),
				'elapsed' => $Task->get('timeElapsed'),
				'remains' => $Task->get('timeRemaining'),
	...
	public function active_multicast()
	{
		...
		// Multicast data
		foreach ($this->FOGCore->getClass('MulticastSessionsManager')->find(array('stateID' => array(1,2,3))) AS $MS)
		{
	...
	public function scheduled()
	{
		...
		foreach ($this->FOGCore->getClass('ScheduledTaskManager')->find() AS $task)
		{
