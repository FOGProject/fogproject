...
class PluginManagementPage extends FOGPage {
    ...
    public function run() {
        $plugin = static::getClass('Plugin',@min(static::getSubObjectIDs('Plugin',array('name'=>$_SESSION['fogactiveplugin']))));
        try {
            ...
            case true:
                ...
                case 'capone':
                    ...
                    array_map(function(&$dmifield) {
                        $checked = static::getSetting('FOG_PLUGIN_CAPONE_DMI') == $dmifield ? ' selected' : '';
                    ...
                    ob_start();
                    printf('<option value="0"%s>%s</option>',(!static::getSetting('FOG_PLUGIN_CAPONE_SHUTDOWN') ? ' selected' : ''),_('Reboot after deploy'));
                    printf('<option value="1"%s>%s</option>',(static::getSetting('FOG_PLUGIN_CAPONE_SHUTDOWN') ? ' selected' : ''),_('Shutdown after deploy'));
