...
class DashboardPage extends FOGPage {
    ...
    public function __construct($name = '') {
        ...
        if (isset($_REQUEST['id'])) $this->obj = static::getClass('StorageNode',$_REQUEST['id']);
        $this->menu = array();
        $this->subMenu = array();
        $this->notes = array();
    }
    public function index() {
        ...
        $SystemUptime = static::$FOGCore->SystemUptime();
        $fields = array(
            _('Username') => $_SESSION['FOG_USERNAME'],
            _('Web Server') => static::getSetting('FOG_WEB_HOST'),
            _('TFTP Server') => static::getSetting('FOG_TFTP_HOST'),
        ...
        static::$HookManager->processEvent('DashboardData',array('data'=>&$this->data,'templates'=>&$this->templates,'attributes'=>&$this->attributes));
        $this->render();
        echo '</li>';
        unset($this->templates,$this->attributes,$fields,$SystemUptime);
        $StorageEnabledCount = static::getClass('StorageNodeManager')->count(array('isEnabled'=>1,'isGraphEnabled'=>1));
        if (static::getClass('StorageNodeManager')->count(array('isEnabled'=>1,'isGraphEnabled'=>1)) > 0) {
            // Activity Pane
            ...
            array_map(function(&$StorageNode) {
                ...
                if (!static::$FOGURLRequests->isAvailable($URL)) return;
                $version = static::$FOGURLRequests->process($URL,'POST');
            ...
            },static::getClass('StorageNodeManager')->find(array('isEnabled'=>1,'isGraphEnabled'=>1)));
    ...
    public function bandwidth() {
        ...
        array_map(function(&$StorageNode) use (&$data) {
            ...
            $URL = filter_var(sprintf('http://%s/%s?dev=%s',$StorageNode->get('ip'),ltrim(static::getSetting('FOG_NFS_BANDWIDTHPATH'),'/'),$StorageNode->get('interface')),FILTER_SANITIZE_URL);
            $dataSet = static::$FOGURLRequests->process($URL,'GET');
            if (!static::$FOGURLRequests->isAvailable($URL)) return;
            unset($URL);
            $data[$StorageNode->get('name')] = json_decode(array_shift($dataSet));
            unset($dataSet,$StorageNode);
        },static::getClass('StorageNodeManager')->find(array('isGraphEnabled'=>1,'isEnabled'=>1)));
    ...
    public function diskusage() {
        try {
            ...
            $Response = static::$FOGURLRequests->process($URL,'GET');
    ...
    public function clientcount() {
        ...
        $StorageGroup = static::getClass('StorageGroup',$this->obj->get('storageGroupID'));
        ...
        array_map(function(&$Node) use (&$ActivityActive,&$ActivityQueued,&$ActivityTotalClients) {
            ...
            if (!static::$FOGURLRequests->isAvailable($URL)) {
            ...
            unset($Node);
        },static::getClass('StorageNodeManager')->find(array('id'=>$StorageGroup->get('enablednodes'))));
        $data = array(
