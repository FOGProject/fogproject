...
abstract class TaskingElement extends FOGBase {
    ...
    protected function ImageLog($checkin = false) {
        if ($checkin === true) return static::getClass('ImagingLog',@max(static::getSubObjectIDs('ImagingLog',array('hostID'=>$this->Host->get('id'),'type'=>$_REQUEST['type'],'complete'=>'0000-00-00 00:00:00'))))
            ...
            ->set('createdBy',$this->Task->get('createdBy'))
            ->save();
        return static::getClass('ImagingLog',@max(static::getSubObjectIDs('ImagingLog',array('hostID'=>$this->Host->get('id')))))
