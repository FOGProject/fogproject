        ...
        try {
            if ($this->imagingTask) {
                if ($this->Task->isMulticast()) {
                    ...
                    $MulticastSession = static::getClass('MulticastSessions',@max(static::getSubObjectIDs('MulticastSessionsAssociation',array('taskID'=>$this->Task->get('id')),'msID')));
                    ...
                    $CheckedInCount = (int)static::getClass('MulticastSessionsAssociationManager')->count(array('msID'=>$MulticastSession->get('id')));
                    ...
                    static::$HookManager->processEvent('TASK_GROUP',array('StorageGroup'=>&$this->StorageGroup,'Host'=>&$this->Host));
                    ...
                    static::$HookManager->processEvent('TASK_NODE',array('StorageNode'=>&$this->StorageNode,'Host'=>&$this->Host));
                } else {
                    if (in_array($this->Task->get('stateID'),array_merge((array)$this->getQueuedState(),(array)$this->getCheckedInState()))) {
                        ...
                        $this->StorageNode = static::nodeFail($this->Image->getStorageGroup()->$method($this->Task->get('imageID')),$this->Host->get('id'));
                        foreach ($this->StorageNodes AS &$StorageNode) {
                            ...
                            if (!isset($tmpStorageNode)) {
                                $tmpStorageNode = static::nodeFail($StorageNode,$this->Host->get('id'));
                                continue;
                            }
                            if ($StorageNode->getClientLoad() < $tmpStorageNode->getClientLoad()) $tmpStorageNode = static::nodeFail($StorageNode,$this->Host->get('id'));
                            unset($StorageNode);
                        }
                    }
            ...
            static::$EventManager->notify('HOST_CHECKIN',array('Host'=>&$this->Host));
            echo '##@GO';
        } catch (Exception $e) {
            echo $e->getMessage();
        }
