					...
					foreach($Image->get('storageGroups') AS $GroupToSend)
					{
						if ($GroupToSend && $GroupToSend->isValid() && $GroupToSend->get('id') != $StorageNode->get('storageGroupID'))
						{
							...
							if ($StorageNodeToSend && $StorageNodeToSend->isValid())
							{
								...
								$process = popen("lftp -e \"set ftp:list-options -a;set net:max-retries 1;set net:timeout 30;".($limit > 0 ? "set net:connection-limit 1;set net:limit-rate 0:$limit;" : '')." mirror -n --ignore-time -R -vvv --exclude 'dev/' --delete $myImage $remImage; exit\" -u $username,$password $ip 2>&1","r");
				...
				if (count($StorageNodeCount) > 0)
				{
					...
					foreach($StorageNodeCount AS $StorageNodeFTP)
					{
						if ($StorageNodeFTP->get('isEnabled'))
						{
							...
							$process = popen("lftp -e \"set ftp:list-options -a;set net:max-retries 1;set net:timeout 30;".($limit > 0 ? "set net:connection-limit 1;set net:limit-rate 0:$limit;" : '')." mirror -n --ignore-time -R -vvv --exclude 'dev/' --delete $myRoot $remRoot; exit\" -u $username,$password $ip 2>&1","r");
							while(!feof($process) && $process != null)
							{
								$output = fgets($process,256);
								if ($output)
