					...
					foreach($Image->get('storageGroups') AS $GroupToSend)
					{
						if ($GroupToSend && $GroupToSend->isValid() && $GroupToSend->get('id') != $StorageNode->get('storageGroupID'))
						{
							...
							if ($StorageNodeToSend && $StorageNodeToSend->isValid())
							{
								...
								$process[$StorageNodeToSend->get('name')] = popen("lftp -e 'set ftp:list-options -a;set net:max-retries 10;set net:timeout 30;$limit mirror --ascii -n -R --ignore-time -P 10 -e -vvv --exclude dev/ --delete-first --scan-all-first $myImage $remImage; exit' -u $username,$password $ip 2>&1","r");
							}
						}
					}
				...
				if (count($StorageNodeCount) > 0)
				{
					...
					foreach($StorageNodeCount AS $StorageNodeFTP)
					{
						if ($StorageNodeFTP->get('isEnabled'))
						{
							...
							if ($limitsend > 0)
								$limit .= "set net:limit-rate 0:$limitsend;";
							$process[$StorageNodeFTP->get('name')] = popen("lftp -e 'set ftp:list-options -a;set net:max-retries 10;set net:timeout 30;$limit mirror --ascii -n -R --ignore-time -P 10 -e -vvv --exclude dev/ --delete-first --scan-all-first $myRoot $remRoot; exit' -u $username,$password $ip 2>&1","r");
						}
					}
					foreach ((array)$process AS $nodename => $proc)
					{
