...
class Group extends FOGController
{
    ...
    public function addImage($imageID)
    {
        ...
        if (self::getClass('TaskManager')->count(array('hostID'=>$this->get('hosts'), 'stateID'=>array_merge($this->getQueuedStates(), (array)$this->getProgressState())))) {
    ...
    public function createImagePackage($taskTypeID, $taskName = '', $shutdown = false, $debug = false, $deploySnapins = false, $isGroupTask = false, $username = '', $passreset = '', $sessionjoin = false, $wol = false)
    {
        ...
        if (self::getClass('TaskManager')->count(array('hostID'=>$hostids, 'stateID'=>array_merge($this->getQueuedStates(), (array)$this->getProgressState())))) {
            throw new Exception(_('There is a host in a tasking'));
        }
        ...
        if ($TaskType->isMulticast()) {
            ...
            if (count($batchTask) > 0) {
                ...
                if (count($multicastsessionassocs) > 0) {
                    self::getClass('MulticastSessionsAssociationManager')->insert_batch(array('msID', 'taskID'), $multicastsessionassocs);
                }
            }
            ...
            if (count($batchTask) > 0) {
                self::getClass('TaskManager')->insert_batch($batchFields, $batchTask);
            }
            unset($hostCount, $hostIDs, $batchTask, $first_id, $affected_rows, $ids, $multicastsessionassocs);
    ...
    private function createSnapinTasking($snapin = -1, $now)
    {
        ...
        if (count($snapinJobs) > 0) {
            ...
            if (count($snapinTasks) > 0) {
                self::getClass('SnapinTaskManager')->insert_batch(array('jobID', 'stateID', 'snapinID'), $snapinTasks);
            }
        }
        return $hostIDs;
    }
