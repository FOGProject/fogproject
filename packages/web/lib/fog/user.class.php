...
class User extends FOGController
{
    ...
    public function password_validate($username, $password, $adminTest = false)
    {
        ...
        if (preg_match('#^[a-f0-9]{32}$#', $tmpUser->get('password')) && md5($password) === $tmpUser->get('password')) {
    ...
    {
        if ($this->key($key) == 'password' && !$override) {
    ...
    private function isLoggedIn()
    {
        ...
        if (!$this->get('authIP') || !$this->get('authUserAgent')) {
            return false;
        } elseif ($this->get('authIP') && $this->get('authIP') != $_SERVER['REMOTE_ADDR']) {
        ...
        } elseif ($this->get('authUserAgent') && $this->get('authUserAgent') != $_SERVER['HTTP_USER_AGENT']) {
            if (!$_SESSION['FOG_MESSAGES']) {
        ...
        } elseif ($this->get('authID') && $this->sessionID != $this->get('authID')) {
            if (!$_SESSION['FOG_MESSAGES']) {
        ...
        } elseif ($this->get('authLastActivity') && !$this->alwaysloggedin && ((time() - $this->get('authLastActivity')) >= ($this->inactivitySessionTimeout*60*60))) {
