...
class SubMenu extends FOGBase {
	...
	private function nodeAndID() {
		...
		foreach((array)$this->subMenu[$this->node][$this->id] AS $link => $menu) {
			if ((string)$menu != 'Array') $this->FOGSubMenu->addItems($this->node,array((string)$menu => (string)$link,),$this->id,$this->name);
		}
	}
	private function nodeOnly() {
		foreach((array)$this->subMenu[$this->node] AS $link => $menu) {
			if ((string)$menu != 'Array') $this->FOGSubMenu->addItems($this->node,array((string)$menu => (string)$link));
		}
	}
	private function buildMenuStruct() {
		if ($this->node && $this->id) $this->nodeAndID();
		else $this->nodeOnly();
		if ($this->title) {
			foreach((array)$this->title AS $title => $item) {
				if((string)$title != 'Array') $this->FOGSubMenu->addNotes($this->node,array((string)$title => (string)$item),$this->id,$this->name);
			}
		}
	...
	private function buildMenuLinks() {
		...
		if ($this->node == 'group') {
			...
			if ($_SESSION['Pending-Hosts'] > 0) $this->subMenu[$this->node]['pending'] = $this->foglang['PendingHosts'];
			...
			if($_REQUEST['id']) {
				...
				if ($Host && $Host->isValid() && !$Host->get('pending')) $this->subMenu[$this->node]['id'][$linkformat.'#host-tasks'] = $this->foglang['BasicTasks'];
			...
			if ($dh != null) {
				while (!(($f=readdir($dh)) === false)) {
					if (is_file($this->FOGCore->getSetting('FOG_REPORT_DIR').$f) && substr($f,strlen($f) - strlen('.php')) === '.php') $this->subMenu[$this->node][$reportlink.base64_encode($f)] = substr($f,0,strlen($f) -4);
				}
			}
