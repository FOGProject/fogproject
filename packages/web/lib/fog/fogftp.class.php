...
class FOGFTP extends FOGGetSet
{
    ...
    public function connect($host = '', $port = 0, $timeout = 90, $autologin = true, $connectmethod = 'ftp_connect')
    {
        try {
            ...
            if (self::$link && self::$currentConnectionHash == self::$lastConnectionHash) {
                return $this;
            }
            if (!$host) {
                $host = $this->get('host');
            ...
            if ((self::$link = $connectmethod($host, $port, $timeout)) === false) {
    ...
    public function login($username = null, $password = null)
    {
        try {
            ...
            if (self::$currentLoginHash == self::$lastLoginHash) {
                return $this;
            }
            if (!$username) {
            ...
            if (ftp_login(self::$link, $username, $password) === false) {
                self::ftperror($this->data);
            }
        } catch (Exception $e) {
            throw new Exception($e->getMessage());
    ...
    public function nb_fput($remote_file, $handle, $mode =0, $startpos = 0)
    {
        ...
        if ($startpos) {
            return ftp_nb_fput(self::$link, $remote_file, $handle, $mode, $startpos);
        }
    ...
    public function nb_put($remote_file, $local_file, $mode =0, $startpos = 0)
    {
        ...
        if ($startpos) {
            return ftp_nb_put(self::$link, $remote_file, $local_file, $mode, $startpos);
        }
    ...
    {
        if (!(ftp_rename(self::$link, $oldname, $newname) || $this->put($newname, $oldname))) {
            self::ftperror($this->data);
        }
        return $this;
    }
