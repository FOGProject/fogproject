...
class Host extends FOGController {
    ...
    public function destroy($field = 'id') {
        ...
        static::getClass('SnapinTaskManager')->destroy(array('jobID'=>$this->getSubObjectIDs('SnapinJob',$find,'id')));
    ...
    private function createSnapinTasking($snapin = -1) {
        try {
            if (static::getClass('SnapinAssociationManager')->count(array('hostID'=>$this->get('id'))) < 1) return;
            ...
            if ($snapin == -1) {
                if (count($this->get('snapins')) < 1) {
                    foreach ((array)static::getClass('SnapinManager')->find() AS $i => &$Snapin) {
    ...
    public function createImagePackage($taskTypeID, $taskName = '', $shutdown = false, $debug = false, $deploySnapins = false, $isGroupTask = false, $username = '', $passreset = '',$sessionjoin = false,$wol = false) {
        try {
            ...
            if ($imagingTypes) {
                ...
                if (!$StorageNode || !$StorageNode->isValid()) throw new Exception(static::$foglang['SGNotValid']);
            ...
            if ($TaskType->isMulticast()) {
                $assoc = false;
                $MultiSessName = current((array)static::getClass('MulticastSessionsManager')->find(array('name'=>$taskName,'stateID'=>array_merge($this->getQueuedStates(),(array)$this->getProgressState()))));
                $MultiSessAssoc = current((array)static::getClass('MulticastSessionsManager')->find(array('image'=>$this->getImage()->get('id'),'stateID'=>0)));
    ...
    public function wakeOnLAN() {
        $URLs = array();
        $mac = $this->getMyMacs();
        $Nodes = static::getClass('StorageNodeManager')->find(array('isEnabled'=>1));
    ...
    public function removeAddMAC($removeArray) {
        foreach((array)$removeArray AS $i => &$item) $this->remove('additionalMACs',static::getClass('MACAddress',$item));
        unset($item);
        return $this;
    }
    public function removePendMAC($removeArray) {
        foreach((array)$removeArray AS $i => &$item) $this->remove('pendingMACs',static::getClass('MACAddress',$item));
        unset($item);
        return $this;
    }
    ...
    public function ignore($imageIgnore,$clientIgnore) {
        ...
        foreach ((array)$imageIgnore AS $i => &$igMAC) {
            $igMAC = static::getClass('MACAddress',$igMAC);
        ...
        foreach ((array)$clientIgnore AS $i => &$cgMAC) {
            $cgMAC = static::getClass('MACAddress',$cgMAC);
    ...
    public function clientMacCheck($MAC = false) {
        return static::getClass('MACAddress',$this->getSubObjectIDs('MACAddressAssociation',array('mac'=>($MAC ? $MAC : $this->get('mac')),'hostID'=>$this->get('id'),'clientIgnore'=>1),'mac'))->isValid() ? 'checked' : '';
    }
    public function imageMacCheck($MAC = false) {
        return static::getClass('MACAddress',$this->getSubObjectIDs('MACAddressAssociation',array('mac'=>($MAC ? $MAC : $this->get('mac')),'hostID'=>$this->get('id'),'imageIgnore'=>1),'mac'))->isValid() ? 'checked' : '';
    }
