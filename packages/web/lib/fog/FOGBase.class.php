...
abstract class FOGBase {
    ...
    protected function array_change_key(&$array, $old_key, $new_key) {
        $array[$new_key] = html_entity_decode(htmlentities(str_replace(array('\r\n','\n'),"\n",mb_convert_encoding($this->DB->sanitize($array[$old_key]),'UTF-8')),ENT_QUOTES,'UTF-8'),ENT_QUOTES,'UTF-8');
        $headersList = headers_list();
        if (!count(preg_grep('#text/plain#',$headersList)) > 0) unset($array[$old_key]);
        else $array[$new_key] = html_entity_decode(stripslashes($array[$new_key]),ENT_QUOTES,'UTF-8');
        return;
    }
