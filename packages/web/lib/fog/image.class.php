...
class Image extends FOGController
{
    ...
    public function getStorageGroup()
    {
        ...
        $primaryGroup = array_filter(array_map(function (&$val) {
            if ($this->getPrimaryGroup($val)) {
                return $val;
            }
            return false;
        }, (array)$this->get('storageGroups')));
        if (count($primaryGroup) < 1) {
            $primaryGroup = (array)@min((array)$this->get('storageGroups'));
        }
    ...
    public function getPrimaryGroup($groupID)
    {
        ...
        if ($primaryCount < 1) {
            $primaryCount = self::getClass('ImageAssociationManager')->count(array('imageID'=>$this->get('id')));
        }
        if ($primaryCount < 1) {
