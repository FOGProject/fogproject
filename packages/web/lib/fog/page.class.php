...
class Page extends FOGBase {
    ...
    public function __construct() {
        parent::__construct();
        if (!$this->theme) {
            $this->theme = static::getSetting('FOG_THEME');
        ...
        $this->isHomepage = (in_array($_REQUEST['node'], array('home', 'dashboard','schemaupdater','client','logout','login')) || !static::$FOGUser->isValid());
        if (static::$FOGUser->isValid() && strtolower($_REQUEST['node']) != 'schemaupdater') {
            if (!static::$isMobile) {
                ...
                if (static::getSetting('FOG_PLUGINSYS_ENABLED')) $this->array_insert_after('about',$this->main,'plugin',array(static::$foglang['Plugin Management'],'fa fa-cog fa-2x'));
            } else {
                $this->main = array(
                    'home'=>array(static::$foglang['Home'],'fa fa-home fa-2x'),
                    'host'=>array(static::$foglang['Host Management'],'fa fa-desktop fa-2x'),
