...
abstract class FOGBase {
    ...
    protected function arrayRemove($key, array &$array) {
        if (is_array($key)) {
            ...
            array_map(function(&$value) use (&$array,&$key) {
                if (is_array($value)) $this->arrayRemove($key, $value);
    ...
    public function formatTime($time, $format = false, $utc = false) {
        if (!$time instanceof DateTime) $time = self::niceDate($time,$utc);
        if ($format) {
    ...
    protected function validDate($Date, $format = '') {
        ...
        if (!$Date instanceof DateTime) $Date = self::niceDate($Date);
    ...
    protected function diff($start, $end, $ago = false) {
        if (!$start instanceof DateTime) $start = self::niceDate($start);
        if (!$end instanceof DateTime) $end = self::niceDate($end);
    ...
    protected function arrayChangeKey(&$array, $old_key, $new_key) {
    ...
    protected function sendData($datatosend,$service = true) {
        if (!$service) return;
        try {
            if (self::niceDate() >= self::niceDate($this->Host->get('sec_time'))) $this->Host->set('pub_key','')->save();
            global $sub;
