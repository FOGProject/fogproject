...
abstract class FOGBase {
    ...
    protected function parseMacList($stringlist,$image = false,$client = false) {
        ...
        if (self::getClass('MACAddressAssociationManager')->count(array('mac'=>$MACs))) $MACs = array_values(array_filter(array_unique(array_map($lowerAndTrim,(array)self::getSubObjectIDs('MACAddressAssociation',array('mac'=>$MACs,'pending'=>0),'mac')))));
        if ($client) {
