...
class Printer extends FOGController {
    ...
    public function save($mainObject = true) {
        ...
        case ($this->isLoaded('hosts')):
            ...
            $Hosts = static::getClass('HostManager')->find(array('id'=>array_diff((array)$this->get('hosts'),(array)$DBHostIDs)));
    ...
    public function updateDefault($hostid,$onoff) {
        foreach ((array)$hostid AS $i => &$id) static::getClass('Host',$id)->updateDefault($this->get('id'),in_array($id,(array)$onoff));
        unset($id);
        return $this;
    }
}
