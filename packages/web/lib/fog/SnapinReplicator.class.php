						...
						foreach($Snapin->get('storageGroups') AS $GroupToSend)
						{
							if ($GroupToSend && $GroupToSend->isValid() && $GroupToSend->get('id') != $StorageNode->get('storageGroupID'))
							{
								...
								if ($StorageNodeToSend && $StorageNodeToSend->isValid())
								{
									...
									$process = popen("lftp -e \"set ftp:list-options -a;set net:max-retries 1;set net:timeout 30;".($limit > 0 ? "set net:connection-limit 1;set net:limit-rate 0:$limit;" : '')." mirror -i $mySnapFile -n --ignore-time -R -vvv --delete $mySnapin $remSnapin; exit\" -u $username,$password $ip 2>&1","r");
						...
						foreach($StorageNodeCount AS $StorageNodeFTP)
						{
							foreach($Snapins AS $Snapin)
							{
								if ($StorageNodeFTP->get('isEnabled'))
								{
									...
									$process = popen("lftp -e \"set ftp:list-options -a;set net:max-retries 1;set net:timeout 30;".($limit > 0 ? "set net:connection-limit 1;set net:limit-rate 0:$limit;" : '')." mirror -i $mySnapFile -n --ignore-time -R -vvv --delete $myRoot $remRoot; exit\" -u $username,$password $ip 2>&1","r");
									while(!feof($process) && $process != null)
									{
										$output = fgets($process,256);
										if ($output)
