							...
							foreach((array)$Snapin->get('storageGroups') AS $GroupToSend)
							{
								...
								if ($GroupToSend && $GroupToSend->isValid() && $GroupToSend->get('id') != $StorageNode->get('storageGroupID'))
								{
									...
									if ($StorageNodeToSend && $StorageNodeToSend->isValid())
									{
										...
										$process[$StorageNodeToSend->get('name')] = popen("lftp -e \"set ftp:list-options -a;set net:max-retries 10;set net:timeout 30;".$limit." mirror --reverse -i $mySnapFile -n --ignore-time -R -vvv --delete-first $mySnapin $remSnapin; exit\" -u $username,$password $ip 2>&1","r");
									}
								}
							}
						}
						...
						foreach($Snapins AS $Snapin)
						{
							...
							foreach($StorageNodeCount AS $StorageNodeFTP)
							{
								if ($StorageNodeFTP->get('isEnabled'))
								{
									...
									if ($limitsend > 0)
										$limit .= "set net:limit-rate 0:$limitsend;";
									$process[$StorageNodeFTP->get('name')] = popen("lftp -e \"set ftp:list-options -a;set net:max-retries 10;set net:timeout 30;".$limit." mirror --reverse -i $mySnapFile -n --ignore-time -R -vvv --delete-first $myRoot $remRoot; exit\" -u $username,$password $ip 2>&1","r");
								}
							}
						}
						foreach((array)$process AS $nodename => $proc)
