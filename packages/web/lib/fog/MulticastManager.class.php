                    ...
                    if($this->isMCTaskNew($KnownTasks, $curTask->getID())) {
                            ...
                            if($curTask->getClientCount() > 0) {
                                ...
                                if(is_numeric($curTask->getPortBase()) && $curTask->getPortBase() % 2 == 0) {
                                    ...
                                    if($curTask->startTask()) {
                                        ...
                                        if($curTask->killTask()) $this->outall(sprintf(" | Task (%s) %s has been cleaned.",$curTask->getID(),$curTask->getName()));
                                        else $this->outall(sprintf(" | Task (%s) %s has NOT been cleaned.",$curTask->getID(),$curTask->getName()));
                                    }
                                } else $this->outall(sprintf(" | Task (%s) %s failed to execute, port must be even and numeric.",$curTask->getID(),$curTask->getName()));
                            } else $this->outall(sprintf(" | Task (%s) %s failed to execute, no clients are included!",$curTask->getID(),$curTask->getName()));
                        } else $this->outall(sprintf(" | Task (%s) %s failed to execute, image file:%s not found!",$curTask->getID(),$curTask->getName(),$curTask->getImagePath()));
                    } else {
                        ...
                        if ($runningTask->isRunning()) {
                            ...
                            if ($jobcancelled || $curSession->get('stateID') == 5) {
                                ...
                                } else $this->outall(sprintf(" | Failed to kill task (%s) %s!",$runningTask->getID(),$runningTask->getName()));
                            } else {
