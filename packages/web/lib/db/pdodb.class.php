...
class PDODB extends DatabaseManager {
    ...
    private function connect() {
        try {
            if (self::$link) return $this;
            self::$link = new PDO(sprintf('%s:host=%s;charset=utf8',DATABASE_TYPE,preg_replace('#^p[:]#','',DATABASE_HOST)),DATABASE_USERNAME,DATABASE_PASSWORD,array(PDO::ATTR_PERSISTENT => true));
            self::$link->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            self::query("SET SESSION sql_mode=''");
            if (!self::current_db($this) && !preg_match('#schemaupdater#',htmlspecialchars($_SERVER['QUERY_STRING'],ENT_QUOTES,'utf-8'))) $this->redirect('?node=schemaupdater');
