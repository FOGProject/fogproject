...
class PersistentGroupsManager extends FOGManagerController
{
    ...
    public function install($name)
    {
        ...
        $sql = 'DELIMITER $$'
            ...
            . 'SET @myTemplateID = '
            . '(SELECT `hostID` FROM `groups` '
            . 'INNER JOIN `hosts` ON '
            . '(`groupName` = `hostName`) WHERE `groupID`=@myGroupID);'
            . 'IF (@myTemplateID IS NOT NULL) AND (@myHostID <> @myTemplateID) THEN'
            . 'UPDATE `hosts` `d`, (SELECT `hostImage`, '
            . '`hostBuilding`, `hostUseAD`, `hostADDomain`, `hostADOU`, '
            . '`hostADUser`, `hostADPass`, `hostADPassLegacy`, '
            . '`hostProductKey`, `hostPrinterLevel`, `hostKernelArgs`, '
            . '`hostExitBios`, `hostExitEfi`, `hostEnforce` '
            . 'FROM `hosts` WHERE `hostID`=@myTemplateID) `s`'
            . 'SET `d`.`hostImage`=`s`.`hostImage`, `d`.`hostBuilding`='
            . '`s`.`hostBuilding`, '
            . '`d`.`hostUseAD`=`s`.`hostUseAD`, `d`.`hostADDomain`='
            . '`s`.`hostADDomain`,'
            . '`d`.`hostADOU`=`s`.`hostADOU`, `d`.`hostADUser`=`s`.`hostADUser`, '
            . '`d`.`hostADPass`=`s`.`hostADPass`, `d`.`hostADPassLegacy`='
            . '`s`.`hostADPassLegacy`,'
            . '`d`.`hostProductKey`=`s`.`hostProductKey`, '
            . '`d`.`hostPrinterLevel`=`s`.`hostPrinterLevel`, `d`.`hostKernelArgs`='
            . '`s`.`hostKernelArgs`,'
            . '`d`.`hostExitBios`=`s`.`hostExitBios`, '
            . '`d`.`hostExitEfi`=`s`.`hostExitEfi`, '
            . '`d`.`hostEnforce`=`s`.`hostEnforce`'
            ...
            . 'INSERT INTO `printerAssoc` '
            . '(`paHostID`,`paPrinterID`,`paIsDefault`,`paAnon1`,'
            . '`paAnon2`,`paAnon3`,`paAnon4`)'
            . 'SELECT @myHostID as `paHostID`,`paPrinterID`,'
            . '`paIsDefault`,`paAnon1`,`paAnon2`,`paAnon3`,`paAnon4`'
