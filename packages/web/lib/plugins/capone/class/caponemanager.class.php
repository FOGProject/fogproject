...
class CaponeManager extends FOGManagerController
{
    ...
    {
        $res = true;
        if (!self::$DB->query("DROP TABLE IF EXISTS `capone`")->fetch()->get()) {
            $res = false;
        }
        if (!self::getClass('ServiceManager')->destroy(array('name'=> 'FOG_PLUGIN_CAPONE_%'))) {
            $res = false;
        }
        if (!self::getClass('PXEMenuOptionsManager')->destroy(array('name' => 'fog.capone'))) {
            $res = false;
        }
        return $res;
    }
