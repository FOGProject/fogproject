...
class LocationManagementPage extends FOGPage {
    ...
    public function index() {
        $this->title = _('Search');
        if ($this->getSetting('FOG_DATA_RETURNED')>0 && static::getClass('LocationManager')->count() > $this->getSetting('FOG_DATA_RETURNED') && $_REQUEST['sub'] != 'list') $this->redirect(sprintf('?node=%s&sub=search',$this->node));
        $this->data = array();
