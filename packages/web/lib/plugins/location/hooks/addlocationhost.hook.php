...
class AddLocationHost extends Hook {
    ...
    public function HostData($arguments) {
        ...
        foreach((array)$arguments['data'] AS $index => &$vals) {
            $locationID = static::getSubObjectIDs('LocationAssociation',array('hostID'=>$arguments['data'][$index]['id']),'locationID');
            $locID = array_shift($locationID);
            $arguments['data'][$index]['location'] = static::getClass('Location',$locID)->get('name');
            unset($vals);
        }
    }
    public function HostFields($arguments) {
        if (!in_array($this->node,(array)$_SESSION['PluginsInstalled'])) return;
        if ($_REQUEST['node'] != 'host') return;
        $locationID = static::getSubObjectIDs('LocationAssociation',array('hostID'=>$arguments['Host']->get('id')),'locationID');
        $locID = array_shift($locationID);
        $this->array_insert_after(_('Host Product Key'),$arguments['fields'],_('Host Location'),static::getClass('LocationManager')->buildSelectBox($locID));
    }
    public function HostAddLocation($arguments) {
        ...
        static::getClass('LocationAssociationManager')->destroy(array('hostID'=>$arguments['Host']->get('id')));
        $Location = static::getClass('Location',$_REQUEST['location']);
        if ($Location->isValid()) $Location->addHost($arguments['Host']->get('id'))->save(false);
    }
    public function HostImport($arguments) {
        if (!in_array($this->node,(array)$_SESSION['PluginsInstalled'])) return;
        $Location = static::getClass('Location',$arguments['data'][5]);
    ...
    public function HostEmailHook($arguments) {
        ...
        $locationID = static::getSubObjectIDs('LocationAssociation',array('hostID'=>$arguments['Host']->get('id')),'locationID');
        $locID = array_shift($locationID);
        if (!static::getClass('Location',$locID)->isValid()) return;
        $this->array_insert_after("\nSnapin Used: ",$arguments['email'],"\nImaged From (Location): ",static::getClass('Location',$locID)->get('name'));
    }
    public function HostRegister($arguments) {
        if (!in_array($this->node,(array)$_SESSION['PluginsInstalled'])) return;
        $locationID = trim(base64_decode($_REQUEST['location']));
        $Location = static::getClass('Location',$locationID);
        if (!$Location->isValid()) return;
        $Location->addHost($arguments['Host']->get('id'))->save(false);
        $Host = $arguments['Host'];
        static::$HookManager->processEvent('HOST_REGISTER_LOCATION',array('Host'=>$Host,'Location'=>&$Location));
    }
}
