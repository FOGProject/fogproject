...
class LocationManager extends FOGManagerController {
    public function install($name) {
        ...
        if (static::$DB->query($sql)->fetch()->get()) {
            ...
            ENGINE=MyISAM";
            if (static::$DB->query($sql)->fetch()->get()) return true;
        }
        return false;
    }
    public function uninstall() {
        $res = true;
        if (!static::$DB->query("DROP TABLE IF EXISTS `locationAssoc`")->fetch()->get()) $res = false;
        if (!static::$DB->query("DROP TABLE IF EXISTS `location`")->fetch()->get()) $res = false;
        return $res;
    }
}
