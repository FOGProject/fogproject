...
class UpdateClient extends FOGClient implements FOGClientSend
{
    ...
    public function send()
    {
        ...
        if (!in_array($action, $this->actions)) {
            throw new Exception('#!er: '._('Needs action string of ask, get, or list'));
        }
        if (in_array($action, $this->fileActions) && !$_REQUEST['file']) {
            throw new Exception('#!er: '._('If action of ask or get, we need a file name in the request'));
        }
        ...
        switch ($action) {
            ...
            case 'get':
                $ClientUpdateFile = array_shift($ClientUpdateFiles);
                if (!($ClientUpdateFile && $ClientUpdateFile->isValid())) {
                    throw new Exception(_('Invalid data'));
                }
