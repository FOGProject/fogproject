        ...
        if (!isset($_REQUEST['exitcode']) && (!isset($_REQUEST['taskid']) || !is_numeric($_REQUEST['taskid']))) {
            ...
            return array(
                'snapins' => array_filter(array_map(function (&$Snapin) use ($date) {
                    if (!$Snapin->isValid()) {
                        return array('error'=>_('Invalid Snapin'));
                    }
                    ...
                    if (!$SnapinTask->isValid()) {
                        return array('error'=>_('Invalid Snapin Tasking'));
                    }
                    $StorageGroup = null;
                    ...
                    if (!$StorageGroup->isValid()) {
                        return array('error'=>_('Invalid Storage Group'));
                    }
                    $StorageNode = null;
                    ...
                    if (!$StorageNode->isValid()) {
                        return array('error'=>_('Invalid Storage Node'));
                    }
                    ...
                    if (!$avail[0]) {
                        return array('error'=>_('No connection to get snapin'));
                    }
                    $SnapinTask
                        ->set('checkin', $date)
                    ...
                    if (empty($hash)) {
                        return array('error'=>_('No hash available'));
                    }
                    if ($size === 0) {
                        return array('error'=>_('No size available'));
                    }
                    return array(
    ...
    public function send()
    {
        ...
        if (!isset($_REQUEST['exitcode']) && (!isset($_REQUEST['taskid']) || !is_numeric($_REQUEST['taskid']))) {
            ...
            if ($this->newService) {
                ...
                $avail = self::$FOGURLRequests->isAvailable($url);
                if (!$avail[0]) {
                    throw new Exception(_('Cannot connect to the Storage Node'));
                }
                $response = self::$FOGURLRequests->process($url, 'POST', array('filepath'=>$filepath));
