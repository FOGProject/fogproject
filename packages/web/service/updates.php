...
try {
    if (!in_array($_REQUEST[action],array('ask','get','list'))) throw new Exception('#!er: Needs action string of ask, get, or list');
    if (in_array($_REQUEST[action],array('ask','get')) && !$_REQUEST['file']) throw new Exception('#!er: If action of ask or get, needs a file name in request');
    else if ($_REQUEST['action'] == 'ask') {
        foreach($FOGCore->getClass(ClientUpdaterManager)->find(array('name' => base64_decode($_REQUEST['file']))) AS $ClientUpdate) $Datatosend = $_REQUEST['newService'] ? "#!ok\n#md5=".$ClientUpdate->get('md5') : $ClientUpdate->get('md5');
    ...
    if ($Data) $Datatosend = $_REQUEST['newService'] ? "#!ok\n".implode("\n",$Data) : implode("\n",$Data);
