...
class HostUsers extends ReportManagementPage {
	...
	public function index() {
		...
		$report->endCSVLine();
		$cnt = 0;
		foreach (self::getClass('HostManager')->find('','','','','','name') AS $i => &$Host) {
            ...
            $report->endCSVLine();
            $cnt1 = 0;
            foreach ((array)self::getClass('UserTrackingManager')->find(array('id'=>$Host->get('users'),'action'=>array(null,0,1)),'','datetime','DESC','','username') AS $i => $User) {
