    ...
    *)
        ...
        [[ -z $packages ]] && {
            if [[ $OSVersion -gt 7 ]]; then
                ...
                packages="attr curl dhcp gcc gcc-c++ genisoimage git gzip lftp m4 make mod_fastcgi mod_ssl mtools mysql mysql-server net-tools nfs-utils openssl php php-cli php-common php-fpm php-gd php-ldap php-mbstring php-mysqlnd php-process syslinux tar tftp-server util-linux vsftpd wget xz-devel"
            fi
        }
