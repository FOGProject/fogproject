 ...
 * only after that post to Post_Stage3, which is what closes the task,
 * records the deploy on the host and writes the Complete task log row. The manager's "is no longer running" arm used to call
 ...
 * found for Host". ackIfAlreadyComplete() then answered '##' for them, so the
 * client moved on, but the checkout itself never ran (forum topic 18254).
 *
 ...
 * database by prove_multicast_checkout_after_sender_exit_16.php, which fails
...
$mgr = mcStrip("$root/packages/web/src/Service/MulticastManager.php");
$sess = mcStrip("$root/packages/web/src/Items/MulticastSession.php");
$queue = mcStrip("$root/packages/web/src/TaskHandling/TaskQueue.php");
...
mcCheck(
    ...
    && false === strpos($complete, "['msID' => \$this->get('id')] );"),
    $failures,
    $checks
);
