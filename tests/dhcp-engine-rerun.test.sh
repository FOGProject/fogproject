...
#
# resolveDHCPEngine() swaps the ISC package for the Kea package in $packages,
# and the installer saves that list. It does not save $dhcpname or $dhcpconfig,
# so the next run re-seeds both from the distro config: the ISC package name and
# the ISC config path. The guard looked only for the ISC name in the saved list.
# Every re-run of a Kea install therefore returned early and kept the ISC path.
# On Ubuntu that wrote the Kea JSON to /etc/dhcp3/dhcpd.conf, AppArmor refused
# kea-dhcp4 the read, and the install stopped before TFTP.
#
...
for key in packages dhcpengine; do
...
reset() { unset dhcpconfig dhcpname keapackage keaservice keaconfig dhcpd dhcpconfigother; }

bldhcp=1
for distro in ubuntu redhat arch; do
    [[ -f $REPO/lib/$distro/config.sh ]] || continue
    echo "$distro:"

    reset; unset dhcpengine; INSTALLED=""
    seed "$distro" || continue
    iscconfig=$dhcpconfig
    iscpkg=$dhcpname
    keapkg=$keapackage
    packages="bc $iscpkg lftp"
    resolveDHCPEngine
    is "$dhcpengine" kea "first run picks Kea"
    ...
    savedengine=$dhcpengine
    reset; INSTALLED="$keapkg"
    packages=$savedpackages
    dhcpengine=$savedengine
    seed "$distro"
    resolveDHCPEngine
    is "$dhcpengine" kea "re-run stays on Kea"
    is "$dhcpconfig" /etc/kea/kea-dhcp4.conf "re-run writes the Kea config path"
    is "$dhcpname" "$keapkg" "re-run names the Kea package"
    is "$packages" "$savedpackages" "re-run leaves the package list alone"

    # An existing ISC install must not move on a re-run.
    reset; INSTALLED="$iscpkg"
    packages="bc $iscpkg lftp"
    dhcpengine=isc
    seed "$distro"
    resolveDHCPEngine
    is "$dhcpengine" isc "ISC re-run stays on ISC"
    is "$dhcpconfig" "$iscconfig" "ISC re-run keeps the ISC config path"
    is "$packages" "bc $iscpkg lftp" "ISC re-run keeps the ISC package"
done
