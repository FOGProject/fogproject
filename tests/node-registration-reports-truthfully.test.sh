...
# "exists", so the registration branch is always the one taken.
cat > "$tmp/harness.sh" <<HARNESS
WEB_url_proto=http
NET_fog_server_ip=127.0.0.1:$port
WEB_root=/
...
NET_interface=eth0
SVC_user=fogproject
SVC_password=hunter2
maxClients=10
_nodeRegistrationName() { echo "testnode"; }
. "$tmp/under-test.sh"
HARNESS

run_mode() {
    echo "$1" > "$tmp/mode"
    bash -c ". '$tmp/harness.sh'; $2" 2>&1
}
