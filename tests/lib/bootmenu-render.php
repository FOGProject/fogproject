...
FOGBase::$settings = array_merge(
    array(
        ...
        // The six falseTasking() destructures positionally; stock values
        // from commons/schema.php. Absent, the harness throws rather than
        // shifting every later list() variable by one.
...
$rows = array(
    ...
    array(13, 'fog.approvehost', 'Approve This Host', '0', '6', null,
        $loginParams('approveHost 1')),
    array(14, 'fog.enrollsecureboot', 'Enroll Secure Boot Key', '0', '2',
        null, ''),
);
...
FOGBase::$HookManager = new StubHookManager(
    (array)(isset($scenario['hookMutations']) ? $scenario['hookMutations'] : array())
);
 ...
 * the CLI SAPI.
 */
FOGBase::$requestMacs = (array)($scenario['requestMacs'] ?? array());
 ...
 */
$menu = new BootMenu();
$falseTasking = (array)($scenario['falseTasking'] ?? array());
if ($falseTasking) {
    ...
    $menu->falseTasking('', (new StubTask(array('image' => $falseTasking)))->getImage());
}
