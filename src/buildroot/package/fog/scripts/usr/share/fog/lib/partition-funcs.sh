...
makeSwapSystem() {
    ...
    local part_type=`sfdisk -d "$disk" 2>/dev/null | egrep "^$2" | awk -F, '{print $3;}' | awk -F= '{print $2;}' | sed -r 's/\s//g'`;
...
processSfdisk()
{
	if [ -f "$imagePath/d1.minimum.partitions" ]; then
		local minstart=`cat "$imagePath/d1.minimum.partitions" | grep ${hd}1 | awk -F'=' '{print $2}' | cut -d',' -f1`;
		if [ "$minstart" == 63 ]; then
			local chunksize=512;
		else
