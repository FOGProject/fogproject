...
case $linuxReleaseName in
    *[Dd][Ee][Bb][Ii][Aa][Nn]*|*[Bb][Uu][Nn][Tt][Uu]*)
        apt-get -yq install lsb_release 2>/var/log/fog_error_${version}.log 2>&1
        ;;
    *[Cc][Ee][Nn][Tt][Oo][Ss]*|*[Rr][Ee][Dd]*[Hh][Aa][Tt]*|*[Ff][Ee][Dd][Oo][Rr][Aa]*)
        command -v dnf 2>/var/log/fog_error_${version}.log 2>&1
        if [[ $? -eq 0 ]]; then
            dnf -y install redhat-lsb-core 2>>/var/log/fog_error_${version}.log 2>&1
        else
            yum -y install redhat-lsb-core 2>>/var/log/fog_error_${version}.log 2>&1
        fi
        ;;
esac
if [[ -z $OSVersion ]]; then
...
command -v systemctl 2>>/var/log/fog_error_${version}.log 2>&1
if [[ $? == 0 ]]; then
    systemctl="yes"
fi
installtype=""
...
grep -l webroot /opt/fog/.fogsettings 2>>/var/log/fog_error_${version}.log 2>&1
            ...
            if [[ $installtype == S ]]; then
                ...
                if [[ -d $backupPath/fog_web_${version}.BACKUP ]]; then
                    if [[ ! -d $backupPath/fogDBbackups ]]; then
                        mkdir -p $backupPath/fogDBbackups 2>>/var/log/fog_error_${version}.log 2>&1
                    fi
                    wget --no-check-certificate -O $backupPath/fogDBbackups/fog_sql_${version}_$(date +"%Y%m%d_%I%M%S").sql "http://$ipaddress/$webroot/management/export.php?type=sqldump" 2>>/var/log/fog_error_${version}.log 2>&1
                fi
                errorStat $?
                if [[ $installtype == N && -z $dbupdate ]]; then
                    ...
                    wget -O - --post-data="confirm=1" --no-proxy http://127.0.0.1/${webroot}management/index.php?node=schemaupdater 2>>/var/log/fog_error_${version}.log 2>&1 ||
                        wget -O - --post-data="confirm=1" --no-proxy http://${ipaddress}/${webroot}management/index.php?node=schemaupdater 2>>/var/log/fog_error_${version}.log 2>&1
                    errorStat $?
                fi
                configureStorage
                configureDHCP
